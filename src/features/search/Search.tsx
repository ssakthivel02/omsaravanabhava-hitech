...
interface Hit { href: string; titleTa: string | null; titleEn: string | null; kind: Exclude<Facet, 'all'>; kindTa: string; kindEn: string; aliases: string[]; metadata: string[]; state?: string | null | undefined; sourceConfidence?: string | null | undefined; }
const PAGE_SIZE = 20;
...
const INDEX: Hit[] = [
  ...
  ...workCatalogue.map((work) => ({ href: work.id ? `/works#work-${work.id}` : '/works', titleTa: work.titleTa, titleEn: work.titleEn, kind: 'work' as const, kindTa: 'நூல்', kindEn: 'Work', aliases: [], metadata: 'author' in work ? [work.author, work.period].filter((value): value is string => Boolean(value)) : [], state: 'rightsState' in work ? work.rightsState : work.verificationState })),
  ...kumarastavam.map((record) => ({ href: `/prayers#k-${record.id}`, titleTa: record.titleTa, titleEn: record.transliteration, kind: 'work' as const, kindTa: 'துதி', kindEn: 'Prayer', aliases: [record.transliteration, record.transliteration?.normalize('NFKD').replace(/\p{M}/gu, '')].filter((value): value is string => Boolean(value)), metadata: [record.author, record.edition].filter((value): value is string => Boolean(value)), state: record.canonicalTextStatus })),
  ...muruganNames.filter((name) => Boolean(name.nameTa || name.nameEn)).map((name, index) => ({ href: `/knowledge#name-${name.id ?? `name-${index + 1}`}`, titleTa: name.nameTa, titleEn: name.nameEn, kind: 'name' as const, kindTa: 'முருகன் பெயர்', kindEn: 'Murugan name', aliases: [], metadata: [name.meaning].filter((value): value is string => Boolean(value)), sourceConfidence: name.sources[0]?.confidence })),
];
function scoreHit(hit: Hit, rawNeedle: string): number { const needle = normalizeSearch(rawNeedle); if (!needle) return 0; const titleTa = normalizeSearch(hit.titleTa ?? ''); const titleEn = normalizeSearch(hit.titleEn ?? ''); const aliases = hit.aliases.map(normalizeSearch); const metadata = hit.metadata.map(normalizeSearch); if (titleTa === needle) return 1000; if (aliases.some((value) => value === needle)) return 950; if (titleEn === needle) return 925; if (titleTa.startsWith(needle)) return 850; if (titleEn.startsWith(needle)) return 825; if (aliases.some((value) => value.startsWith(needle))) return 800; if (titleTa.includes(needle)) return 700; if (titleEn.includes(needle)) return 675; if (aliases.some((value) => value.includes(needle))) return 650; if (metadata.some((value) => value.includes(needle))) return 300; return 0; }
const FACETS: Array<{ value: Facet; labelTa: string; labelEn: string }> = [
  { value: 'all', labelTa: 'அனைத்தும்', labelEn: 'All' }, { value: 'temple', labelTa: 'கோயில்கள்', labelEn: 'Temples' }, { value: 'arupadai', labelTa: 'அறுபடை வீடு', labelEn: 'Six Abodes' }, { value: 'thiruppugazh', labelTa: 'திருப்புகழ்', labelEn: 'Thiruppugazh' }, { value: 'work', labelTa: 'நூல்கள்', labelEn: 'Works' }, { value: 'name', labelTa: 'முருகன் பெயர்கள்', labelEn: 'Murugan names' },
];
...
export default function Search() {
  ...
  return (
    <article className="page search-page">
      ...
      <section aria-labelledby={resultsId}><h2 id={resultsId} className="sr-only">{text('தேடல் முடிவுகள்', 'Search results')}</h2><ul className="temple-list search-results">{hits.map((hit) => { const sourceState = hit.sourceConfidence ? describeSourceConfidence(hit.sourceConfidence, locale) : null; const showEnglishFirst = locale === 'en' && Boolean(hit.titleEn); return <li key={`${hit.kind}:${hit.href}:${hit.titleTa ?? hit.titleEn ?? ''}`}><Link href={hit.href} className="temple-row search-result-row">{showEnglishFirst ? <><b lang="en">{hit.titleEn}</b>{hit.titleTa && <small lang="ta">{hit.titleTa}</small>}</> : <><b lang={hit.titleTa ? 'ta' : 'en'}>{hit.titleTa ?? hit.titleEn}</b>{hit.titleEn && <small>{hit.titleEn}</small>}</>}<em className="tag" lang={locale}>{text(hit.kindTa, hit.kindEn)}</em><span className="search-result-states">{sourceState && <span className={`state state-${sourceState.tone}`}><span className="state-dot" aria-hidden="true" /><span lang={locale}>{sourceState.label}</span></span>}{hit.state && <StateBadge state={hit.state} />}</span></Link></li>; })}</ul></section>
      {shown < allHits.length && <button type="button" className="btn btn-quiet" onClick={() => setShown((value) => value + PAGE_SIZE)}><span lang={locale}>{text('மேலும் காட்டு', 'Show more')}</span></button>}
    </article>
  );
}
