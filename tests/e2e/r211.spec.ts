...
test.describe('R2.11 Thiruppugazh catalogue browser qualification', () => {
  ...
  for (const [number, opening] of samples) {
    test(`publishes validated song ${number} metadata and keeps its canonical body withheld`, async ({ page }) => {
      ...
      await expect(page.getByText('காட்டப்படுவது 1 / 600')).toBeVisible();
      await song.click();
  ...
  test('publishes validated song 600 metadata and keeps its canonical body withheld', async ({ page }) => {
    await page.goto('/thiruppugazh');
    await page.getByLabel('திருப்புகழ் தேடல்').fill('அத் துகிரின் நல்');
    const songSixHundred = page.getByRole('link', { name: /அத் துகிரின் நல்.*600/ });
    await expect(songSixHundred).toBeVisible();
    await expect(page.getByText('காட்டப்படுவது 1 / 600')).toBeVisible();
    await songSixHundred.click();
    await expect(page).toHaveURL(/thiruppugazh-0600/);
    await expect(page.getByText(/உரை.*ஏற்றப்படவில்லை|text.*not.*imported/i).first()).toBeVisible();
  });

  test('filters to exactly the 270 governed Part II records', async ({ page }) => {
    await page.goto('/thiruppugazh');
    await page.getByLabel('மூல பகுதி').selectOption('part-2');
    await expect(page.getByText('காட்டப்படுவது 270 / 600')).toBeVisible();
    await expect(page.getByRole('link', { name: /அற்றைக் கற்றை.*331/ })).toBeVisible();
    await expect(page.getByRole('link', { name: /அத் துகிரின் நல்.*600/ })).toBeVisible();
  });
  ...
  test('does not invent still-uncovered Part III records', async ({ page }) => {
    ...
    await expect(page.getByText('காட்டப்படுவது 0 / 600')).toBeVisible();
  });
