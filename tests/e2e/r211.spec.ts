...
test.describe('R2.11 Thiruppugazh catalogue browser qualification', () => {
  ...
  for (const [number, opening] of samples) {
    test(`publishes validated song ${number} metadata and keeps its canonical body withheld`, async ({ page }) => {
      ...
      await expect(page.getByText('காட்டப்படுவது 1 / 330')).toBeVisible();
      await song.click();
  ...
  test('publishes validated song 330 metadata and keeps its canonical body withheld', async ({ page }) => {
    await page.goto('/thiruppugazh');
    await page.getByLabel('திருப்புகழ் தேடல்').fill('முட்டுப் பட்டு');
    const songThreeHundredThirty = page.getByRole('link', { name: /முட்டுப் பட்டு.*330/ });
    await expect(songThreeHundredThirty).toBeVisible();
    await expect(page.getByText('காட்டப்படுவது 1 / 330')).toBeVisible();
    await songThreeHundredThirty.click();
    await expect(page).toHaveURL(/thiruppugazh-0330/);
    await expect(page.getByText(/உரை.*ஏற்றப்படவில்லை|text.*not.*imported/i).first()).toBeVisible();
  });
  ...
  test('searches verified entries without inventing missing songs', async ({ page }) => {
    ...
    await expect(page.getByText('காட்டப்படுவது 0 / 330')).toBeVisible();
  });
