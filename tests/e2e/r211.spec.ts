...
test.describe('R2.11 Thiruppugazh catalogue browser qualification', () => {
  ...
  for (const [number, opening] of samples) {
    test(`publishes validated song ${number} metadata and keeps its canonical body withheld`, async ({ page }) => {
      ...
      await expect(page.getByText('காட்டப்படுவது 1 / 300')).toBeVisible();
      await song.click();
  ...
  test('publishes validated song 300 metadata and keeps its canonical body withheld', async ({ page }) => {
    await page.goto('/thiruppugazh');
    await page.getByLabel('திருப்புகழ் தேடல்').fill('வார் உற்று எழும்');
    const songThreeHundred = page.getByRole('link', { name: /வார் உற்று எழும்.*300/ });
    await expect(songThreeHundred).toBeVisible();
    await expect(page.getByText('காட்டப்படுவது 1 / 300')).toBeVisible();
    await songThreeHundred.click();
    await expect(page).toHaveURL(/thiruppugazh-0300/);
    await expect(page.getByText(/உரை.*ஏற்றப்படவில்லை|text.*not.*imported/i).first()).toBeVisible();
  });
  ...
  test('searches verified entries without inventing missing songs', async ({ page }) => {
    ...
    await expect(page.getByText('காட்டப்படுவது 0 / 300')).toBeVisible();
  });
