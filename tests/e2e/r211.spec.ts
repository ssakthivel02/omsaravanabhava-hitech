...
test.describe('R2.11 Thiruppugazh catalogue browser qualification', () => {
  ...
  for (const [number, opening] of samples) {
    test(`publishes validated song ${number} metadata and keeps its canonical body withheld`, async ({ page }) => {
      ...
      await expect(page.getByText('காட்டப்படுவது 1 / 800')).toBeVisible();
      await song.click();
  ...
  test('publishes validated song 800 metadata and keeps its canonical body withheld', async ({ page }) => {
    await page.goto('/thiruppugazh');
    await page.getByLabel('திருப்புகழ் தேடல்').fill('சூழ்ந்து ஏன்ற துக்க');
    const songEightHundred = page.getByRole('link', { name: /சூழ்ந்து ஏன்ற துக்க.*800/ });
    await expect(songEightHundred).toBeVisible();
    await expect(page.getByText('காட்டப்படுவது 1 / 800')).toBeVisible();
    await songEightHundred.click();
    await expect(page).toHaveURL(/thiruppugazh-0800/);
    await expect(page.getByText(/உரை.*ஏற்றப்படவில்லை|text.*not.*imported/i).first()).toBeVisible();
  });
  ...
  test('filters to exactly the 340 governed Part II records', async ({ page }) => {
    ...
    await expect(page.getByText('காட்டப்படுவது 340 / 800')).toBeVisible();
  ...
  test('filters to exactly the 130 governed Part III records', async ({ page }) => {
    await page.goto('/thiruppugazh');
    await page.getByLabel('மூல பகுதி').selectOption('part-3');
    await expect(page.getByText('காட்டப்படுவது 130 / 800')).toBeVisible();
    await expect(page.getByRole('link', { name: /பரவி உனது.*671/ })).toBeVisible();
    await expect(page.getByRole('link', { name: /சூழ்ந்து ஏன்ற துக்க.*800/ })).toBeVisible();
  });
