...
describe('R2.11 Thiruppugazh corpus catalogue', () => {
  it('locks the governed runtime at exactly 750 source-linked records with no canonical bodies', () => {
    expect(thiruppugazh).toHaveLength(750);
    ...
    expect(thiruppugazh.at(-1)?.id).toBe('thiruppugazh-0750');
    expect(thiruppugazh.at(-1)?.openingWords).toBe('குடத் தாமரையாம்');
  });

  it('distinguishes the 1326-song reference corpus from the 750 source-linked records', () => {
  ...
  for (const [number, opening] of samples) {
    it(`publishes validated metadata through song ${number}`, async () => {
      ...
      expect(screen.getByText('காட்டப்படுவது 1 / 750')).toBeInTheDocument();
    });
  }

  it('publishes validated Part III metadata through song 750 without publishing its body', async () => {
    const user = userEvent.setup();
    renderAt('/thiruppugazh');
    await user.type(screen.getByLabelText('திருப்புகழ் தேடல்'), 'குடத் தாமரையாம்');
    const songSevenHundredFifty = screen.getByRole('link', { name: /குடத் தாமரையாம்/ });
    expect(songSevenHundredFifty).toHaveAttribute('href', '/thiruppugazh/thiruppugazh-0750');
    expect(screen.getByText('காட்டப்படுவது 1 / 750')).toBeInTheDocument();
  });
  ...
  it('exposes exactly the 340 governed Part II records currently promoted', async () => {
    ...
    expect(screen.getByText('காட்டப்படுவது 340 / 750')).toBeInTheDocument();
  ...
  it('exposes exactly the 80 governed Part III records currently promoted', async () => {
    const user = userEvent.setup();
    renderAt('/thiruppugazh');
    await user.selectOptions(screen.getByLabelText('மூல பகுதி'), 'part-3');
    expect(screen.getByText('காட்டப்படுவது 80 / 750')).toBeInTheDocument();
    expect(screen.getByRole('link', { name: /பரவி உனது/ })).toHaveAttribute('href', '/thiruppugazh/thiruppugazh-0671');
    expect(screen.getByRole('link', { name: /குடத் தாமரையாம்/ })).toHaveAttribute('href', '/thiruppugazh/thiruppugazh-0750');
  });
