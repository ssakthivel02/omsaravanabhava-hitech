...
describe('R2.11 Thiruppugazh corpus catalogue', () => {
  it('locks the governed runtime at exactly 850 source-linked records with no canonical bodies', () => {
    expect(thiruppugazh).toHaveLength(850);
    ...
    expect(thiruppugazh.at(-1)?.id).toBe('thiruppugazh-0850');
    expect(thiruppugazh.at(-1)?.openingWords).toBe('இதசந்தன புழுகு');
  });

  it('distinguishes the 1326-song reference corpus from the 850 source-linked records', () => {
  ...
  for (const [number, opening] of samples) {
    it(`publishes validated metadata through song ${number}`, async () => {
      ...
      expect(screen.getByText('காட்டப்படுவது 1 / 850')).toBeInTheDocument();
    });
  }

  it('publishes validated Part III metadata through song 850 without publishing its body', async () => {
    const user = userEvent.setup();
    renderAt('/thiruppugazh');
    await user.type(screen.getByLabelText('திருப்புகழ் தேடல்'), 'இதசந்தன புழுகு');
    const songEightHundredFifty = screen.getByRole('link', { name: /இதசந்தன புழுகு/ });
    expect(songEightHundredFifty).toHaveAttribute('href', '/thiruppugazh/thiruppugazh-0850');
    expect(screen.getByText('காட்டப்படுவது 1 / 850')).toBeInTheDocument();
  });
  ...
  it('exposes exactly the 340 governed Part II records currently promoted', async () => {
    ...
    expect(screen.getByText('காட்டப்படுவது 340 / 850')).toBeInTheDocument();
  ...
  it('exposes exactly the 180 governed Part III records currently promoted', async () => {
    const user = userEvent.setup();
    renderAt('/thiruppugazh');
    await user.selectOptions(screen.getByLabelText('மூல பகுதி'), 'part-3');
    expect(screen.getByText('காட்டப்படுவது 180 / 850')).toBeInTheDocument();
    expect(screen.getByRole('link', { name: /பரவி உனது/ })).toHaveAttribute('href', '/thiruppugazh/thiruppugazh-0671');
    expect(screen.getByRole('link', { name: /இதசந்தன புழுகு/ })).toHaveAttribute('href', '/thiruppugazh/thiruppugazh-0850');
  });
