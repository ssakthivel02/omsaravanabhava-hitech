...
describe('R2.11 Thiruppugazh corpus catalogue', () => {
  it('distinguishes the 1326-song reference corpus from the 100 source-linked records', () => {
  ...
  it('publishes validated batch 002 metadata through song 30 without publishing its body', async () => {
    ...
    expect(screen.getByText('காட்டப்படுவது 1 / 100')).toBeInTheDocument();
  });
  ...
  it('does not fabricate records for an uncovered source part', async () => {
    ...
    expect(screen.getByText('காட்டப்படுவது 0 / 100')).toBeInTheDocument();
  });
