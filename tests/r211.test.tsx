...
describe('R2.11 Thiruppugazh corpus catalogue', () => {
  it('locks the governed runtime at exactly 1325 source-linked records with no canonical bodies', () => {
    expect(thiruppugazh).toHaveLength(1325);
    ...
    expect(thiruppugazh.at(-1)?.id).toBe('thiruppugazh-1325');
    expect(thiruppugazh.at(-1)?.openingWords).toBe('உரையுஞ் சென்றது');
  });

  it('distinguishes the 1326-song reference corpus from the 1325 source-linked records', () => {
  ...
  for (const [number, opening] of samples) {
    it(`publishes validated metadata through song ${number}`, async () => {
      ...
      expect(screen.getByText('காட்டப்படுவது 1 / 1325')).toBeInTheDocument();
    });
  }
  ...
  it('publishes validated Part IV metadata through song 1275 without publishing its body', async () => {
    const user = userEvent.setup();
    renderAt('/thiruppugazh');
    await user.type(screen.getByLabelText('திருப்புகழ் தேடல்'), 'மூலா நிலமதின்');
    const songOneThousandTwoHundredSeventyFive = screen.getByRole('link', { name: /மூலா நிலமதின்/ });
    expect(songOneThousandTwoHundredSeventyFive).toHaveAttribute('href', '/thiruppugazh/thiruppugazh-1275');
    expect(screen.getByText('காட்டப்படுவது 1 / 1325')).toBeInTheDocument();
  });

  it('publishes validated Part IV metadata through song 1300 without publishing its body', async () => {
    const user = userEvent.setup();
    renderAt('/thiruppugazh');
    await user.type(screen.getByLabelText('திருப்புகழ் தேடல்'), 'புத்தகத்து ஏட்டில்');
    const songOneThousandThreeHundred = screen.getByRole('link', { name: /புத்தகத்து ஏட்டில்/ });
    expect(songOneThousandThreeHundred).toHaveAttribute('href', '/thiruppugazh/thiruppugazh-1300');
    expect(screen.getByText('காட்டப்படுவது 1 / 1325')).toBeInTheDocument();
  });

  it('publishes validated Part IV metadata through song 1325 without publishing its body', async () => {
    const user = userEvent.setup();
    renderAt('/thiruppugazh');
    await user.type(screen.getByLabelText('திருப்புகழ் தேடல்'), 'உரையுஞ் சென்றது');
    const songOneThousandThreeHundredTwentyFive = screen.getByRole('link', { name: /உரையுஞ் சென்றது/ });
    expect(songOneThousandThreeHundredTwentyFive).toHaveAttribute('href', '/thiruppugazh/thiruppugazh-1325');
    expect(screen.getByText('காட்டப்படுவது 1 / 1325')).toBeInTheDocument();
  });
  ...
  it('exposes exactly the 340 governed Part II records currently promoted', async () => {
    ...
    expect(screen.getByText('காட்டப்படுவது 340 / 1325')).toBeInTheDocument();
  ...
  it('exposes exactly the 330 governed Part III records currently promoted', async () => {
    ...
    expect(screen.getByText('காட்டப்படுவது 330 / 1325')).toBeInTheDocument();
  ...
  it('exposes exactly the 325 governed Part IV records currently promoted', async () => {
    const user = userEvent.setup();
    renderAt('/thiruppugazh');
    await user.selectOptions(screen.getByLabelText('மூல பகுதி'), 'part-4');
    expect(screen.getByText('காட்டப்படுவது 325 / 1325')).toBeInTheDocument();
