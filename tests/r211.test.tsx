...
describe('R2.11 Thiruppugazh corpus catalogue', () => {
  it('locks the governed runtime at exactly 1025 source-linked records with no canonical bodies', () => {
    expect(thiruppugazh).toHaveLength(1025);
    ...
    expect(thiruppugazh.at(-1)?.id).toBe('thiruppugazh-1025');
    expect(thiruppugazh.at(-1)?.openingWords).toBe('சீதமலம் வெப்பு');
  });

  it('distinguishes the 1326-song reference corpus from the 1025 source-linked records', () => {
  ...
  for (const [number, opening] of samples) {
    it(`publishes validated metadata through song ${number}`, async () => {
      ...
      expect(screen.getByText('காட்டப்படுவது 1 / 1025')).toBeInTheDocument();
    });
  }
  ...
  it('exposes exactly the 340 governed Part II records currently promoted', async () => {
    ...
    expect(screen.getByText('காட்டப்படுவது 340 / 1025')).toBeInTheDocument();
  ...
  it('exposes exactly the 330 governed Part III records currently promoted', async () => {
    ...
    expect(screen.getByText('காட்டப்படுவது 330 / 1025')).toBeInTheDocument();
  ...
  it('exposes exactly the 25 governed Part IV records currently promoted', async () => {
    const user = userEvent.setup();
    renderAt('/thiruppugazh');
    await user.selectOptions(screen.getByLabelText('மூல பகுதி'), 'part-4');
    expect(screen.getByText('காட்டப்படுவது 25 / 1025')).toBeInTheDocument();
    expect(screen.getByRole('link', { name: /இலகி யிருகுழை/ })).toHaveAttribute('href', '/thiruppugazh/thiruppugazh-1001');
    expect(screen.getByRole('link', { name: /சீதமலம் வெப்பு/ })).toHaveAttribute('href', '/thiruppugazh/thiruppugazh-1025');
  });
