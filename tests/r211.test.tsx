...
describe('R2.11 Thiruppugazh corpus catalogue', () => {
  it('distinguishes the 1326-song reference corpus from the 200 source-linked records', () => {
  ...
  it('publishes validated batch 002 metadata through song 30 without publishing its body', async () => {
    ...
    expect(screen.getByText('காட்டப்படுவது 1 / 200')).toBeInTheDocument();
  });
  ...
  it('publishes validated batch 009 metadata through song 200 without publishing its body', async () => {
    const user = userEvent.setup();
    renderAt('/thiruppugazh');
    await user.type(screen.getByLabelText('திருப்புகழ் தேடல்'), 'வேய் இசைந்து');
    const songTwoHundred = screen.getByRole('link', { name: /வேய் இசைந்து/ });
    expect(songTwoHundred).toHaveAttribute('href', '/thiruppugazh/thiruppugazh-0200');
    expect(screen.getByText('காட்டப்படுவது 1 / 200')).toBeInTheDocument();
  });
  ...
  it('does not fabricate records for an uncovered source part', async () => {
    ...
    expect(screen.getByText('காட்டப்படுவது 0 / 200')).toBeInTheDocument();
  });
