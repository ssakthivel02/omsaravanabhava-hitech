...
describe('R2.11 Thiruppugazh corpus catalogue', () => {
  it('locks the governed runtime at exactly 600 source-linked records with no canonical bodies', () => {
    expect(thiruppugazh).toHaveLength(600);
    ...
    expect(thiruppugazh.at(-1)?.id).toBe('thiruppugazh-0600');
    expect(thiruppugazh.at(-1)?.openingWords).toBe('அத் துகிரின் நல்');
  });

  it('distinguishes the 1326-song reference corpus from the 600 source-linked records', () => {
  ...
  for (const [number, opening] of samples) {
    it(`publishes validated metadata through song ${number}`, async () => {
      ...
      expect(screen.getByText('காட்டப்படுவது 1 / 600')).toBeInTheDocument();
    });
  }

  it('publishes validated Part II metadata through song 600 without publishing its body', async () => {
    const user = userEvent.setup();
    renderAt('/thiruppugazh');
    await user.type(screen.getByLabelText('திருப்புகழ் தேடல்'), 'அத் துகிரின் நல்');
    const songSixHundred = screen.getByRole('link', { name: /அத் துகிரின் நல்/ });
    expect(songSixHundred).toHaveAttribute('href', '/thiruppugazh/thiruppugazh-0600');
    expect(screen.getByText('காட்டப்படுவது 1 / 600')).toBeInTheDocument();
  });

  it('exposes exactly the 270 governed Part II records currently promoted', async () => {
    const user = userEvent.setup();
    renderAt('/thiruppugazh');
    await user.selectOptions(screen.getByLabelText('மூல பகுதி'), 'part-2');
    expect(screen.getByText('காட்டப்படுவது 270 / 600')).toBeInTheDocument();
    expect(screen.getByRole('link', { name: /அற்றைக் கற்றை/ })).toHaveAttribute('href', '/thiruppugazh/thiruppugazh-0331');
    expect(screen.getByRole('link', { name: /அத் துகிரின் நல்/ })).toHaveAttribute('href', '/thiruppugazh/thiruppugazh-0600');
  });
  ...
  it('does not fabricate records for the still-uncovered Part III lane', async () => {
    ...
    expect(screen.getByText('காட்டப்படுவது 0 / 600')).toBeInTheDocument();
  });
