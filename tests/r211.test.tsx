...
describe('R2.11 Thiruppugazh corpus catalogue', () => {
  it('locks the governed runtime at exactly 1175 source-linked records with no canonical bodies', () => {
    expect(thiruppugazh).toHaveLength(1175);
    ...
    expect(thiruppugazh.at(-1)?.id).toBe('thiruppugazh-1175');
    expect(thiruppugazh.at(-1)?.openingWords).toBe('பாணிக்கு உட்படாது');
  });

  it('distinguishes the 1326-song reference corpus from the 1175 source-linked records', () => {
    renderAt('/thiruppugazh');
    expect(screen.getByText('1326')).toBeInTheDocument();
    expect(screen.getByText('1175')).toBeInTheDocument();
  ...
  for (const [number, opening] of samples) {
    it(`publishes validated metadata through song ${number}`, async () => {
      ...
      expect(screen.getByText('காட்டப்படுவது 1 / 1175')).toBeInTheDocument();
    });
  }
  ...
  it('exposes exactly the 340 governed Part II records currently promoted', async () => {
    ...
    expect(screen.getByText('காட்டப்படுவது 340 / 1175')).toBeInTheDocument();
  ...
  it('exposes exactly the 330 governed Part III records currently promoted', async () => {
    ...
    expect(screen.getByText('காட்டப்படுவது 330 / 1175')).toBeInTheDocument();
  ...
  it('exposes exactly the 175 governed Part IV records currently promoted', async () => {
    const user = userEvent.setup();
    renderAt('/thiruppugazh');
    await user.selectOptions(screen.getByLabelText('மூல பகுதி'), 'part-4');
    expect(screen.getByText('காட்டப்படுவது 175 / 1175')).toBeInTheDocument();
    expect(screen.getByRole('link', { name: /இலகி யிருகுழை/ })).toHaveAttribute('href', '/thiruppugazh/thiruppugazh-1001');
    expect(screen.getByRole('link', { name: /பாணிக்கு உட்படாது/ })).toHaveAttribute('href', '/thiruppugazh/thiruppugazh-1175');
  });
